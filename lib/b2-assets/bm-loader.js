jQuery(document).ready(function($) {
  ...
  $("#build_bookmarklet").click(function(e) {
    ...
    let sref = $('#sref_description').val();

    if (id === '') { alert('Please enter Skimlinks ID'); return; }
    if ($item.val() === '') { alert('Please select Advertiser'); return; }

    if (sref !== '') {
      link = `https://go.skimresources.com?id=${id}&mid=${mid}&sref=${sref}&murl=`;
    } else {
      link = `https://go.skimresources.com?id=${id}&mid=${mid}&murl=`;
    }
  ...
  $("#build_url").click(function(e) {
    ...
    if (sref !== '') {
      link = `https://go.skimresources.com?id=${id}&mid=${mid}&sref=${sref}&murl=${murl}`;
    } else {
      link = `https://go.skimresources.com?id=${id}&mid=${mid}&murl=${murl}`;
    }
